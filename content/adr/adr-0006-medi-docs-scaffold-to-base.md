...
- 2026-05-01 (II): **모델 정정 — plugin = installer only, runtime = .claude/**. 사용자 피드백: plugin runtime 에서 hook/SKILL 발동하면 사용자 가시성 0 + git 추적 X + 팀 공유 X. 또한 commands/ 사장 announce → 자연어 description-trigger SKILL 만 사용. 정정 내용: (1) base/hooks/hooks.json 에서 H1·H2 제거 (글로벌 H4·H5 만 유지), (2) base/role-templates/base/ 신설 (role-templates/<role>/ 패턴 따름) — 3 SKILL 풀셋 (medi-new / medi-version-cut / docs-validate) × 5 자산 (SKILL.md + rules.md + checklist.md + examples/ + scripts/), hooks/hooks.json (H1·H2), scripts/ (medi-scaffold-check, medi-claude-md-augment, medi-claude-md-uninstall) — 모두 init 시 .claude/ 로 복사, (3) base/role-templates/base/role.json — `auto_include: true` 마커, init.sh 가 모든 init 호출 시 base 자동 포함, (4) base/commands/medi-*.md 폐기 — SKILL description-trigger 로 통합, (5) init.sh 확장 — base auto-include + role-templates 의 scripts/ → .claude/scripts/ 복사 + augment 호출, (6) medi-docs-templates/CLAUDE.md.snippet 의 slash-list 마커 → skill-list 마커, (7) medi-claude-md-augment.sh 동적 introspection 모드 — `.claude/skills/*/SKILL.md` 의 name+description 추출해 합성 (정적 snippet 조각 폐기), (8) role-templates/<role>/CLAUDE.md.snippet 7개 폐기. /tmp/medi-init-smoke 에서 새 init 흐름 검증 완료 — base auto-include + 3 SKILL + backend 5 SKILL + hooks/scripts → .claude/ + scaffold + augment 본문 보존.
