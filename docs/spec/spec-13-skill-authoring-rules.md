...
모든 SKILL.md (메인테이너 도구 `.claude/skills/*` + 사용자 배포본 `content/harness/plugins/*/skills/*`) 의 frontmatter 스키마 + 본문 규칙 + 보안 정책 + 검증 룰셋. 작성 형식·검증 룰은 두 영역 동일, **권한 강도만 차등** (메인테이너용은 신뢰 기반으로 `allow_commands` 자유, 사용자 배포본은 엄격). 메타·콘텐츠 docs (`docs/idea`, `docs/spec`, `docs/adr`) 의 frontmatter 는 [[spec-03-frontmatter-naming]] 가 다룸 — 검증 대상이 다름.
...
### 위험 명령 분류 기준 (`allow_commands`)

기준: **현재 셸 환경 외부에 영향 + 되돌리기 어려움**. read-only 는 안전, write/external/destructive 는 위험.

| 카테고리 | 예시 | 분류 |
|----------|------|------|
| 파일 시스템 destructive | `rm`, `mv`, `chmod`, `chown` | **위험 — 화이트리스트 필수** |
| 네트워크 호출 | `curl`, `wget`, `nc`, `ssh` | **위험** (외부 영향) |
| VCS write | `git push`, `git tag`, `git reset --hard`, `git commit` | **위험** |
| 패키지 install | `npm install`, `pip install`, `brew install` | **위험** (외부 의존 도입) |
| 시스템 권한 | `sudo`, `su` | **위험** |
| read-only | `ls`, `cat`, `echo`, `grep`, `find`, `git status`, `git log`, `git diff` | **안전 — 화이트리스트 불필요** |

영역별 차등:
- **메인테이너용 SKILL.md** — 위험 명령 자유롭게 선언 (예: `promote-docs` 의 `git tag`).
- **사용자 배포본 SKILL.md** — 위험 명령 최소화. 사용자 환경 손상 위험이라 *꼭 필요한 명령만* 선언, 본문에 사용 이유 명시.

### `scripts/sanitize.sh` 헬퍼 책임 범위

공용 헬퍼로 **`.claude/skills/scripts/sanitize.sh` 단일 파일 안 함수 라이브러리** 형태로 제공한다. 스킬이 `source scripts/sanitize.sh` 후 필요 함수만 호출.

| 함수 | 책임 |
|------|------|
| `sanitize_slug <input>` | kebab-case 슬러그화 (공백·특수문자 제거 + 정규화) |
| `validate_kebab_case <input>` | 입력이 `^[a-z0-9]+(-[a-z0-9]+)*$` 패턴인지 검증 |
| `quote_arg <input>` | 셸 인용 (`printf '%q'`) — 인젝션 방지 |
| `validate_path <input>` | 경로 탈출 (`../`) / 절대경로 / 심볼릭 검증 |

헬퍼 사용은 강제하지 않고, 본문 가이드에서 사용을 권장하는 방식으로 노출. 단일 파일이라 distribution 단순, 함수별 분리로 확장 가능.
...
운영 후 결정 (ADR 화 시점에 ADR Follow-ups 로 이월):

- [ ] `reads_files` / `runs_scripts` / `env_vars` 권장 → 강제 전환 시점 — 운영 6개월 후 위반 사례 누적 측정.
- [ ] `validate.py` 일반화 시 SKILL.md 타깃 추가가 기존 docs 검증 성능에 영향 — 운영 시점 측정.
