...
| **R9** status 값 enum | type 별 enum: spec `draft/active/accepted/deprecated`, adr (메타·콘텐츠 공통) `proposed/accepted/superseded/deprecated`, inbox `pending/promoted/archived`, sources `pending/promoted/superseded`, wiki `pending/promoted/superseded`, harness `pending/released`. status 자체는 콘텐츠 단계에서 *선택* (미박은 자산은 통과) | **차단** (값이 enum 외일 때만) |
...
- 2026-04-29: status proposed → accepted. source [[spec-03-frontmatter-naming]] status → accepted (통째 흡수).
- 2026-04-30: R9 status enum 콘텐츠 단계 추가 (inbox/sources/wiki/harness). content/adr 는 메타 adr enum 공유. 자산 작업 추적 (어디까지 처리됐는지) 자연 메커니즘 — 3000+ 자산 확장 대비.
- 2026-04-30: `categories` 어휘 모든 type 의 *선택* 필드 (자유 enum, 다중). 자산 *작업 종류* 인덱싱·필터링 차원 — [[adr-0003-content-pipeline]] §categories. 운영 후 enum 강제 follow-up (R 룰셋 강화).
- 2026-04-30: `role` 어휘 content/adr 의 *권장* 필드 (단일 값, enum: `base|planner|pm|frontend|backend|qa|infra` — [[adr-0011-base-hoisting]] §1). ADR 의 *결정 단위* 가 "자산 → 어느 plugin (role)" 이므로 plugin 매핑 facet. categories (topic facet) 와 직교 — 같은 wiki 자산이 nuance 에 따라 다른 role 별 ADR 로 갈라질 수 있음 (예: code-review topic → backend ADR + frontend ADR). meta-layer ADR (`docs/adr/`) 에는 N/A. 운영 누적 후 R10 enum 검증 룰 follow-up (validate.py).
