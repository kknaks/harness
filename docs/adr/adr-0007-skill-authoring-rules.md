...
- [ ] `create-skill.sh` 도입 — S5–S7 강제 결정으로 우선순위 ↑. 신규 SKILL scaffold 가 4 필수 자산 (SKILL.md + rules.md + examples/sample-no-reference.md + checklist.md) 자동 박음. (reference 로드 SKILL 은 `sample-with-reference.md` 추가 — Notes 2026-04-30 IV §체크리스트 §A)
...
- 2026-04-30 (II): backend `code-review` SKILL 입주 (commit `49c5cc9`) review 7건을 *generator 결함* 으로 진단 → `create-skill.sh` scaffold heredoc 갱신. **보편 슬롯 (모든 SKILL 강제)**: (1) SKILL.md `## 보안 고려사항` 의 시크릿 마스킹 패턴 표 — dotenv / 시크릿 디렉토리 / 토큰 / 키 자료 / 인증 헤더값 5 카테고리 + 정규식 (P2-7), (2) rules.md / checklist.md 의 SSOT 분리 docstring — rules = 본질·룰, checklist = 운영 절차 — 같은 표 양쪽 박지 말 것 (P1-4). **조건부 슬롯 (해당 SKILL 만)** 은 scaffold 박지 않고 `.claude/skills/promote-docs/rules.md` §"Skill / Wiki 합성 시 조건부 슬롯 체크리스트" 로 라우팅 — A. reference 로드 SKILL: rules.md `## reference 로드 모델` (우선순위·충돌 룰·fallback 명시·리포트 출처 강제) + examples/ 두 개 (with-ref / no-ref) (P0-1, P2-6), B. phase 표 SKILL: 헤더에 강제·가이드 분리 (P1-5), C. 산출 포맷 SKILL: 이슈 ID scope (단일/누적/PR) 명시 (P0-2). 미해당 시 슬롯 박지 말기 (죽은 § 회피).
- 2026-04-30 (III): P0-3 (slash command trigger 표준 슬롯) **결정: 본문 `## How to invoke` § 가 SSOT**. 사실 기반 — 기존 4 SKILL (`code-review` / `docs-naming` / `docs-validate` / `promote-docs`) 전부 본문 `## How to invoke` 만 쓰고 frontmatter `commands:` 필드 미사용. Claude Code SKILL 모델 자체가 `description` 매칭 + 별도 plugin manifest (또는 `.claude/commands/*.md`) 로 trigger 바인딩 — frontmatter 에 `commands:` 추가는 사내 확장으로 *비표준*. 현 scaffold 가 이미 박는 § 가 표준 슬롯. plugin manifest 매핑 사례 누적 시 별도 ADR 분기 (지금은 N/A — `.claude/commands/*.md` 도 운영 사례 0).
- 2026-04-30 (IV): **재발 방지 인프라 + retrofit 완료**:
  - [x] `sync-skill.sh <target> [--apply]` 도입 — `.claude/skills/promote-docs/scripts/sync-skill.{sh,py}`. scaffold heredoc 의 *보편 슬롯* (SKILL.md 보안 § + rules/checklist SSOT docstring) 만 idempotent + additive 동기화. 마커 문자열 detect → 기존 사용자 본문 보존. 조건부 슬롯 (reference / phase / 출력 포맷 / examples 두 개) 은 *다루지 않음* — `promote-docs/rules.md` §"Skill / Wiki 합성 시 조건부 슬롯 체크리스트" 가 합성자 책임. `create-skill.sh` 의 "already exists" 에러 메시지에 `sync-skill.sh` 사용법 안내 추가.
  - [x] backend `code-review` SKILL retrofit 완료 — sync-skill 으로 보편 슬롯 cascade + 조건부 슬롯 4건 손 적용 (P0-1 reference 충돌 룰 + 리포트 출처 강제, P0-2 이슈 ID scope = 단일 리포트 unique, P1-5 4단계 표 강제/가이드 컬럼 분리, P2-6 examples 두 개 — `sample-with-reference.md` 신규 + 기존 `sample.md` → `sample-no-reference.md` rename). wiki-01-code-review-pattern §리포트 포맷 에도 ID scope 박음 (P0-2 wiki side).
  - [ ] validate.py R12+ — 시크릿 마스킹 § / `## reference 로드 모델` 존재 (조건부) / 이슈 ID scope 명시 (조건부). 본문 grep 의 한계로 현재는 soft 경고 후보. 운영 사례 누적 후 강제 전환. **sync-skill 의 marker 문자열을 R12 detect 키로 재사용 가능** — 이미 idempotent detect 검증됨.
  - [ ] test-design SKILL 입주 (`content/wiki/wiki-02-test-design-pattern` → `content/adr/adr-0002-test-design-to-backend` 는 이미 작성, 입주는 미수행) — 갱신된 `create-skill.sh` 로 작성하면 보편 슬롯 자동 충족 + 조건부 (reference 로드 / phase 표 / 출력 포맷 여부) 는 본 ADR §체크리스트 통과. *generator 갱신의 첫 검증 케이스*.
  - **원칙 박제** — 조건부 슬롯 (P0-1 / P0-2 / P1-5 / P2-6) 의 retrofit 손 적용은 *합성자 책임 영역의 동기 실행* 으로 정당화. 다음부터는 신규 SKILL 작성 시 `promote-docs/rules.md` §체크리스트 통과로 *예방* — retrofit 케이스 (이미 입주된 SKILL/wiki 의 조건부 누락) 는 본 ADR 의 first-application 학습으로 박제하고, *content/* 직접 patch 의 license 는 열지 않는다 (보편 슬롯은 `sync-skill` 으로, 조건부 슬롯은 합성자가 작성 시점에 통과 — 사후 retrofit 은 사고 후 학습).
