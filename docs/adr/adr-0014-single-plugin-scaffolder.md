...
- 2026-05-01 (V): **합성자 함정 학습 — frontmatter `name:` 카운트 룰**. backend-rules inbox (4 파일 중 3 개가 자체 SKILL `name:` 보유) 를 단일 wiki / ADR / SKILL 로 잘못 묶음. 사용자가 catch — *이전 inbox 2건은 1 SKILL 씩이라 우연히 정합* 했고, 합성자 (Claude) 가 그 패턴을 잘못 일반화해서 "1 inbox = 1 SKILL" 로 박음. 메타-내러티브 ("백엔드 라이프사이클 4단계") 를 발명해서 합리화. 룰·검증 보강 박음:
  - **inbox-to-sources.sh** — 디렉토리 입력 시 child 의 `name:` 카운트 stderr 진단 (block X, mechanical 1:1 유지). 합성자 (사용자·LLM) 에게 *N>1 이면 N wiki 박을 것* 안내.
  - **promote-docs/rules.md §sources→wiki** — "메타-내러티브 (lifecycle / 시리즈) 묶음 금지" + "`name:` 카운트 = wiki 카운트" 룰 추가. 정당한 merge 는 wiki frontmatter `aliases: [merge-justified-<reason>]` 로 명시 사유 박는 escape hatch.
  - **validate.py R13 (`check_wiki_sources_name_count`)** — wiki 의 sources 가 N>1 SKILL 인용하면 violation (`wiki-multi-skill-merge`). PostToolUse 훅 자동 검출. `aliases:` override 인정.
  - **잘못된 산출물 cleanup** — wiki-03 / adr-0003 / role-templates/backend/skills/backend-rules/ 삭제. sources-03 / inbox/backend-rules/ 는 immutable 유지 (재승격 가능). sources-03 status: promoted → pending 리셋.
  - 다음 단계 — sources-03 재승격 시 *3 wiki* (api-design / tdd-cycle / refactor-layered) 박는 게 정합. backend.md (`name:` 없음 = SKILL 아님) 는 reference 동반 문서로 처리 (각 SKILL 의 rules.md §reference 로드 에 인용).
- 2026-04-30 (IV): **manifest 자동화 — 300+ SKILL 야간 작업 대비 도구 3개**. 메인테이너가 매번 JSON 손으로 안 만지게:
  - `.claude/skills/promote-docs/scripts/sync-role-manifest.{sh,py}` — `role-templates/<role>/{skills,commands,hooks}/` 디렉토리 스캔 → role.json 의 배열 자동 재생성. description / version / depends_on 같은 *scalar* 필드 보존. role.json 부재 시 skeleton 생성. `--all` 로 전 role 일괄. 디렉토리에 mv/rm/cp 마음껏 → manifest 따라옴.
  - `.claude/skills/promote-docs/scripts/add-role-skill.sh <role> <skill> [desc]` — create-skill.sh + sync-role-manifest 한 번에. 1 SKILL 박을 때 1 명령. 일괄 시 for-loop 으로 N개.
  - `.claude/skills/docs-validate/scripts/validate.py R12` — manifest drift 검증. 두 방향: (a) manifest 가 선언했으나 디렉토리 부재 (`skill-missing`), (b) 디렉토리 있으나 manifest 에 부재 (`skill-orphan` — 메시지에 sync 명령 박힘). PostToolUse hook 으로 docs/ 또는 content/ 변경 시 자동 → 손으로 잊고 commit 시도 즉시 잡힘.
  - 운영 흐름: `mv/rm/cp` → `sync-role-manifest.sh --all` → `validate.sh` (R12 통과 확인) → `git commit`. 또는 1개씩이면 `add-role-skill.sh <role> <skill>` 한 번. JSON 손 안 댐.
